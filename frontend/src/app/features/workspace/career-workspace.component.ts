...
type Fit = { score:number; decision:string; matchedRequirements:{skill:string;source:string;detail:string}[]; missingRequirements:{skill:string;action:string}[]; nextAction:string; cvAttached:boolean };
...
export class CareerWorkspaceComponent implements OnInit {
  ...
  saveOpportunity(){if(!this.opportunityForm.title.trim()||!this.opportunityForm.company.trim()||!this.opportunityForm.description.trim())return;this.saving=true;this.http.post<Opportunity>(`${api}/opportunities`,this.opportunityForm).subscribe({next:()=>{this.opportunityForm={title:'',company:'',opportunityType:'internship',url:'',description:'',requirements:''};this.saving=false;this.load();},error:()=>{this.error='Could not save this opportunity.';this.saving=false;}});}
